    ...
    <nav className="navbar navbar-expand-lg navbar-dark bg-dark">
      <div className="container d-flex justify-content-between align-items-center">
        ...
        <form className="d-flex my-2 position-relative" onSubmit={(e) => { e.preventDefault(); handleSearch(); }} style={{ flexGrow: 1 }}>
          <input className="form-control" style={{ width: '415px' }} type="search" aria-label="Search" value={searchQuery} onChange={handleInputChange}
            placeholder={searchQuery ? searchQuery : "Search for Movies, Events, Plays, Sports and Activities"}  />
          <button className="btn btn-outline-light ms-2" type="button" onClick={handleSearch}> Search </button>
          {suggestions.length > 0 && (
            <ul className="list-group position-absolute mt-5" style={{ zIndex: 10, width: '420px' }}>
              {loading ? (<li className="list-group-item">Loading...</li>) : (suggestions.slice(0, 5).map((suggestion) => (
                <li key={suggestion.id} className="list-group-item" onClick={() => {
                  setSearchQuery(suggestion.title);
                  navigate(`/movies?query=${suggestion.title}`); setSuggestions([]);
                }}>{suggestion.title}
                </li>
              )))}
            </ul>
        ...
        <div className={`collapse navbar-collapse ${isOpen ? "show" : ""}`} id="navbarNav">
          ...
          <div className="collapse navbar-collapse ms-auto d-flex align-items-center me-2">
            ...
            <div className="d-flex">
              {loggedIn ? (<Link to="/profile" className="btn btn-primary ms-2">Profile</Link>)
