...
const SportsDetails = () => {
  const { id } = useParams(); // Get the sports event ID from the URL
  ...
  useEffect(() => {
    const fetchEventDetails = async () => {
      try {
        setLoading(true);
        const response = await axios.get(`https://api.themoviedb.org/3/movie/${id}?api_key=${apiKey}&language=en-IN`);
        setEvent(response.data);

        // Get cast details
        const castResponse = await axios.get(`https://api.themoviedb.org/3/movie/${id}/credits?api_key=${apiKey}`);
          ...
          <div className="container d-flex align-items-center flex-column flex-lg-row">
            ...
            <div className="position-relative me-3 mb-3 mb-lg-0" style={{ width: '261px', height: '392px' }}>
              <div className="card text-center bg-dark" style={{ padding: '0.1px', backgroundColor: '#171717', width: '261px', height: '392px' }}>
                <img className="card-img-top"  src={event.poster_path ? `https://image.tmdb.org/t/p/w500/${event.poster_path}` : "https://via.placeholder.com/200x300?text=No+Image"} alt={`${event.title} poster`} style={{ width: '100%', height: '360px', objectFit: "center" }} />
                <p className="text-light"> {event.release_date && !isNaN(new Date(event.release_date).getTime())
                  ? new Date(event.release_date) < new Date() ? `Released on ${new Date(event.release_date).toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: 'numeric' })}`
                  : `Upcoming Event on ${new Date(event.release_date).toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: 'numeric' })}`: 'Release Date Unavailable'}
                </p>
            ...
            <div className="text-white text-center text-lg-start" style={{ maxWidth: '500px' }}>
              <h1 className="text-warning mb-3">{event.title}</h1>
              <div className="btn btn-dark mb-2"style={{cursor:'default'}}> <i className="text-danger px-1 bi bi-star-fill cursor-text"></i>
                <span className="px-1 cursor-text">{rating && votes ? `${rating}/10` : 'Not Rated'}</span>
                <span className="px-1 cursor-text">{votes ? `(${votes} votes)` : ''}</span>
              </div>
              <p><strong >Genres:</strong> {event.genres?.map((genre) => genre.name).join('/') || 'Action'}</p> 
              <p><strong>Release Date:</strong> {event.release_date && !isNaN(new Date(event.release_date).getTime())
                ? new Date(event.release_date).toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: 'numeric' })
                : 'Unavailable'}
              </p>
