  ...
  return (
    <div className="movie" style={{ minHeight: '100vh' }}>
      <section className="mb-4" style={{ height: '480px' }}>
        <div className="py-4 mx-auto d-flex align-items-center" style={{
          backgroundImage: `linear-gradient(to right, rgba(0, 0, 0, 1), rgba(0, 0, 0, 0)), url(${backDropUrl})`, backgroundRepeat: "no-repeat",
          backgroundSize: "cover", backgroundPosition: "center", height: "100%", backgroundAttachment: "fixed", color: 'white',
        }}>
          <div className="container d-flex align-items-center" style={{ height: '416px' }}>
            {/* Image div */}
            <div className="position-relative me-3" style={{ width: '261px', height: '392px' }}>
              <div className="card text-center" style={{ padding: '0.1px', backgroundColor: '#171717', width: '261px', height: '392px' }}>
                <img className="card-img-top" src={`https://image.tmdb.org/t/p/w500/${movieDetails.poster_path}`} alt={`${movieDetails.title} poster`} style={{ width: '100%', height: '360px', objectFit: "cover" }} />
                <button onClick={handleShowModal}
                  className="btn btn-info position-absolute top-50 start-50 translate-middle px-4 py-2"
                  style={{ fontSize: "16px", backgroundColor: "rgba(0, 0, 0, 0.7)", color: "white", border: "none", borderRadius: "20px", zIndex: 10, transition: "transform 0.3s ease, background-color 0.3s ease" }}
      ...
      <div className="container">
        ...
        <div className="d-flex flex-wrap">
          {topCast.map((actor) => (
            <div key={actor.id} className="me-2 d-flex flex-column text-center align-items-center">
      ...
      <div className="container">
        ...
        <div className="d-flex flex-wrap">{uniqueCrew.slice(0, 8).map((crewMember) => (
          <div key={crewMember.id} className="me-2 d-flex flex-column text-center align-items-center">
            <img className="rounded-pill me-2 " src={`https://image.tmdb.org/t/p/w200/${crewMember.profile_path}`} alt={crewMember.name} style={{ width: "100px", height: "100px", objectFit: "center", }} />
            <p className="mt-1">{crewMember.name}</p>
            <p className="text-muted small">{crewMember.jobs.map((job, index) => (<span key={index}>{job}{(index % 2 !== 1 && index !== crewMember.jobs.length - 1) ? ", " : ""}{index % 2 === 1 && <br />}</span>))}</p>
          </div>))}
        </div>
      </div>
      {/*Modal */}
